...
const GameListCard = ({ game }) => {
  ...
  return (
    ...
    >
      {game.winner && <CircleCheck className="absolute top-2 right-2 text-green-500" />}
      ...
      <div className="grid grid-cols-[40%_auto_40%] w-full items-center">
        <div className="flex flex-col gap-y-2 justify-center items-center">
            <div className="rounded-[20px] bg-gradient-to-bl  from-[#FFA9CC] via-[#FEB2A4] to-[#FFC36B] w-16 h-16 flex justify-center items-center">
                {game.winner && (game.winner._id === game.teams[0]._id ? (<img src={CrownIcon} className="w-10 h-10 object-cover animate-bounce" />):<img src={EggIcon} className="w-8 h-8 object-cover" />)}
            </div>
        ...
        <div className="flex flex-col gap-y-2  justify-center items-center">
            <div className="rounded-[20px]  bg-gradient-to-br from-[#3385D9] via-[#1BA9EB] to-[#0AC2F8]  w-16 h-16 flex justify-center items-center">
                {game.winner && (game.winner._id === game.teams[1]._id ? (<img src={CrownIcon} className="w-10 h-10 object-cover animate-bounce" />):<img src={EggIcon} className="w-8 h-8 object-cover" />)}
